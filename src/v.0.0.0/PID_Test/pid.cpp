 ...
 */
void pid_Init(int16_t p_factor, int16_t i_factor, int16_t d_factor, int16_t S_Factor, pidData_t *pid)
...
int16_t pid_Controller(int16_t setPoint, int16_t processValue, pidData_t *pid_st)
{
	...
	ret = ( p_term + i_term + d_term ) / pid_st->S_Factor;
	if( ret > MAX_INT ) {
		ret = MAX_INT;
	} else {
		if( ret < -MAX_INT ) {
